...
function ha_apply_advanced_filters_to_wp_search($query)
{
	...
	// Support advs param when s is not present.
	if (empty($query->get('s')) && !empty($get['advs'])) {
		$query->set('s', sanitize_text_field($get['advs']));
	}

	// post_type=post,page
	if (!empty($get['post_type'])) {
		$raw_post_types = explode(',', sanitize_text_field($get['post_type']));
		$post_types = array_values(array_filter(array_map('sanitize_key', $raw_post_types)));
		if (!empty($post_types)) {
			$query->set('post_type', $post_types);
		}
	}

	// Count from widget params.
	if (!empty($get['initial_results_count'])) {
		$query->set('posts_per_page', max(1, (int) $get['initial_results_count']));
	} elseif (!empty($get['per_page'])) {
		$query->set('posts_per_page', max(1, (int) $get['per_page']));
	}

	// Category or page selection.
	$raw_category = !empty($get['category']) ? sanitize_text_field($get['category']) : '';
	$page_id = !empty($get['page_id']) ? absint($get['page_id']) : 0;

	if (!$page_id && strpos($raw_category, 'page:') === 0) {
		$page_id = absint(substr($raw_category, 5));
	}

	if ($page_id > 0) {
		$query->set('post_type', 'page');
		$query->set('page_id', $page_id);
	} elseif (!empty($raw_category) && $raw_category !== 'all') {
		if (strpos($raw_category, 'cat:') === 0) {
			$raw_category = substr($raw_category, 4);
		}
		$query->set('category_name', sanitize_title($raw_category));
	}

	// Date filter.
	$date_mode = !empty($get['date_mode']) ? sanitize_text_field($get['date_mode']) : 'all';
	$from = !empty($get['date_from']) ? sanitize_text_field($get['date_from']) : '';
	$to   = !empty($get['date_to']) ? sanitize_text_field($get['date_to']) : '';
	...
	if ($date_mode !== 'all') {
		...
		switch ($date_mode) {
			case 'today':
				$date_query['year']  = (int) wp_date('Y');
				$date_query['month'] = (int) wp_date('m');
				$date_query['day']   = (int) wp_date('d');
				break;
			case 'yesterday':
				$date_query['year']  = (int) wp_date('Y', strtotime('-1 day'));
				$date_query['month'] = (int) wp_date('m', strtotime('-1 day'));
				$date_query['day']   = (int) wp_date('d', strtotime('-1 day'));
				break;
			case 'last-7-days':
				$date_query['after']  = wp_date('Y-m-d', strtotime('-6 days'));
				$date_query['before'] = wp_date('Y-m-d');
				break;
			case 'this_month':
				$date_query['year']  = (int) wp_date('Y');
				$date_query['month'] = (int) wp_date('m');
				break;
			case 'last-month':
				$date_query['year']  = (int) wp_date('Y', strtotime('first day of last month'));
				$date_query['month'] = (int) wp_date('m', strtotime('first day of last month'));
				break;
			case 'custom':
				if (!empty($from)) {
...
add_action('pre_get_posts', 'ha_apply_advanced_filters_to_wp_search');

/**
 * Render native WP search using selected Elementor template from URL.
 * Example: /?s=hello&result_template_id=123
 */
function ha_render_selected_elementor_search_template()
{
	...
	$template_id = isset($_GET['result_template_id']) ? absint($_GET['result_template_id']) : 0;
	if (!$template_id) {
		return;
	}
	...
	// Allow only published Elementor library templates.
...
add_action('template_redirect', 'ha_render_selected_elementor_search_template', 20);



function ha_extend_posts_search_sql($search, $wp_query)
{
	global $wpdb;
	...
	$is_native_search = (bool) $wp_query->is_search();
	$is_advanced_search = (bool) $wp_query->get('ha_advanced_search');
	$is_ha_search = $is_advanced_search || isset($_GET['result_template_id']) || !empty($_GET['advs']);
			...
			OR REPLACE(
				REGEXP_REPLACE(
					REGEXP_REPLACE(
						...
						'<[sS][tT][yY][lL][eE][^>]*>[[:space:][:print:]]*?</[sS][tT][yY][lL][eE]>',
						''
					),
					'<[^>]+>',
					' '
...
add_filter('posts_search', 'ha_extend_posts_search_sql', 9999, 2);
